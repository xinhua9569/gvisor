...
func getSockOptVal(t *kernel.Task, level, optname uint64, addr usermem.Addr, length usermem.Addr, maximumBlobSize uint, rval uintptr) string {
	if int(rval) < 0 {
	...
	return sockOptVal(t, level, optname, addr, uint64(l), maximumBlobSize)
}

func sockOptVal(t *kernel.Task, level, optname uint64, addr usermem.Addr, length uint64, maximumBlobSize uint) string {
	switch length {
	case 1:
		var v uint8
...
var sockOpts = map[uint64]struct {
	name     string
	optNames map[uint64]string
}{
	linux.SOL_IP: {"SOL_IP", map[uint64]string{
		linux.IP_TTL:                    "IP_TTL",
	...
	linux.SOL_SOCKET: {"SOL_SOCKET", map[uint64]string{
	...
	linux.SOL_TCP: {"SOL_TCP", map[uint64]string{
	...
	linux.SOL_UDP: {"SOL_UDP", map[uint64]string{}},
	linux.SOL_IPV6: {"SOL_IPV6", map[uint64]string{
	...
	linux.SOL_ICMPV6:  {"SOL_ICMPV6", map[uint64]string{}},
	linux.SOL_RAW:     {"SOL_RAW", map[uint64]string{}},
	linux.SOL_PACKET:  {"SOL_PACKET", map[uint64]string{}},
	linux.SOL_NETLINK: {"SOL_NETLINK", map[uint64]string{}},
}

func sockLevel(t *kernel.Task, level uint64) string {
	l, ok := sockOpts[level]
	if !ok {
		return hexNum(uint64(level))
	}
	return l.name
}

func sockOptName(t *kernel.Task, level, optname uint64) string {
	l, ok := sockOpts[level]
	if !ok {
		return hexNum(optname)
	}
	o, ok := l.optNames[optname]
	if !ok {
		return hexNum(optname)
	}
	return o
